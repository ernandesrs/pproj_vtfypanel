                        ...
                        <v-window-item value="general">
                            <v-card variant="flat">
                                <v-card-item>
                                    ...
                                    <v-select v-model="paymentMethods.form.data.method_preferred"
                                        label="Forma de pagamento preferencial" :items="(Object.entries(paymentMethods.methods)).map((method) => {
                                            return {
                                                value: method[0],
                                                text: method[1]
                                            }
                                    ...
                                    <v-select v-model="paymentMethods.form.data.preferred_card_id"
                                        label="Cartão preferencial" :items="paymentMethods.form.data.cards.map((card) => {
                                            return {
                                                value: card.id,
                                ...
                                <v-card-item class="text-center">
                                    <v-btn @click.stop="methodUpdatePaymentMethod" prepend-icon="mdi-check" text="Atualizar"
                                        color="primary" :loading="paymentMethods.form.submitting"
                                        :disabled="paymentMethods.form.submitting"></v-btn>
                                </v-card-item>
                            </v-card>
                        </v-window-item>
                                ...
                                <v-card-item class="mb-5">
                                    <v-row justify="center">
                                        <template v-if="paymentMethods.form.data.cards.length">
                                            <v-col v-for="theCard in paymentMethods.form.data.cards" :key="theCard.id"
                                                cols="12" sm="6" xl="4" class="py-5">
                                                <div class="mb-4"><span class="font-weight-medium">Nome do cartão:</span> {{
                                                    theCard.name }}</div>
                                                <v-card :elevation="card.form.data?.id == theCard.id ? 4 : 2" border
                                                    @click.stop="methodEditCard" :data-identificator="theCard.id"
                                                    class="card-card"
                                                    :color="card.form.data?.id == theCard.id ? 'primary' : ''">
                                                    <v-card-item>
                                                        <div class="d-none">
                                                            <tiny-text-elem text="Nome do cartão" size="small"
                                                                muted></tiny-text-elem>
                                                            <p class="text-h7">{{ theCard.name }}</p>
                                                        </div>
                                                        <div>
                                                            <tiny-text-elem text="Número do cartão" size="small"
                                                                muted></tiny-text-elem>
                                                            <p class="text-h7">{{ theCard.secure_number }}</p>
                                                        </div>
                                                        <div class="d-flex justify-between">
                                                            <div>
                                                                <tiny-text-elem text="Bandeira" size="small"
                                                                    muted></tiny-text-elem>
                                                                <p class="text-h7">{{ theCard.brand.toUpperCase() }}</p>
                                                            </div>

                                                            <div class="mx-6">
                                                                <tiny-text-elem text="Validade" size="small"
                                                                    muted></tiny-text-elem>
                                                                <p class="text-h7">{{ theCard.expiration_date }}</p>
                                                            </div>

                                                            <div>
                                                                <tiny-text-elem text="CVV" size="small"
                                                                    muted></tiny-text-elem>
                                                ...
                                                <div v-if="card.form.data?.id == theCard.id" class="text-center">
                                                    <confirmation-button icon="mdi-trash-can-outline" text="Excluir"
                                                        variant="text" size="small" color="danger"
                                                        dialog-title="Excluir cartão selecionado?"
