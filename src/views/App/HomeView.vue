<template>
  ...
  <template v-else>
    <v-sheet v-if="!computed_userApp.isVerified || !hasActiveSubscription" class="py-5">
      <v-row>
        <v-col v-if="!computed_userApp.isVerified" cols="12">
...
export default {
  ...
  methods: {
    method_resendLink() {
      if (this.computed_userApp.isVerified) {
        return;
      }
